...
function ENT:Initialize()
	...
	self.health = gHeroin.Config.Entities.EntityHealth["ammoina"]
end
...
function ENT:StartTouch(ent)
	...
	if(ent:GetClass() == "gheroin_mixer") then
		...
		if(ent:HasIngredient("ammonia") == true) then return end
		if(ent:ReadyForIngredient("ammonia") == false) then return end
		-- Register that it has Heroin in it, and remove the ent.
		ent:AddIngredient("ammonia")
		self:Remove()
	end
end